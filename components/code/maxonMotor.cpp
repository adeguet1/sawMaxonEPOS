#include <sawMaxonEPOS/maxonMotor.h>
...
// #include "bigssMath/bigss_debug_util.h"
...
maxonMotor::maxonMotor(const unsigned short id, HANDLE c, const std::string &taskName) :
  ...
  stopEndTime(-1.0),
  setPointMode(false),
  lastVelocityCommand(mtsComponentManager::GetInstance()->GetTimeServer().GetRelativeTime() * 1000.0), // in ms
...
void maxonMotor::Run()
{
  startTime = mtsComponentManager::GetInstance()->GetTimeServer().GetRelativeTime() * 1000.0; // in ms
  ...
  if (setPointMode)
  {
    ...
    bool timedOut = (mtsComponentManager::GetInstance()->GetTimeServer().GetRelativeTime() * 1000.0) - lastVelocityCommand > lastVelocityCommandThresh;
  ...
  endTime = mtsComponentManager::GetInstance()->GetTimeServer().GetRelativeTime() * 1000.0; // in ms
}
...
{
  stopStartTime = mtsComponentManager::GetInstance()->GetTimeServer().GetRelativeTime() * 1000.0; // in ms
  ...
  stopEndTime = mtsComponentManager::GetInstance()->GetTimeServer().GetRelativeTime() * 1000.0; // in ms
}
...
{
  setVelocityStartTime = mtsComponentManager::GetInstance()->GetTimeServer().GetRelativeTime() * 1000.0; // in ms

  targetVelocity = std::abs(unitsToRPM(vel.Data));
  setPositionProfile();

  setVelocityEndTime = mtsComponentManager::GetInstance()->GetTimeServer().GetRelativeTime() * 1000.0; // in ms
}
...
void maxonMotor::jogPlus()
{
  ...
  moveVelocityStartTime = mtsComponentManager::GetInstance()->GetTimeServer().GetRelativeTime() * 1000.0; // in ms
  VCS_MoveWithVelocity(controller, nodeId, targetVelocity, &errorCode);
  moveVelocityEndTime = mtsComponentManager::GetInstance()->GetTimeServer().GetRelativeTime() * 1000.0; // in ms
}
